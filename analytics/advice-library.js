(function (global) {
  var adviceLibrary = [
    ...
    },
    {
      ...
      kind: "interrupt",
      habits: ["social"],
      triggerTags: ["fatigue", "ritual", "stress"],
      timeSegments: ["evening", "night"],
  ...
  function normalizeTriggers(triggerTags) {
    return Array.isArray(triggerTags) ? triggerTags : [];
  }

  function matchHabit(habits, habitId) {
    if (habits.indexOf(habitId) >= 0) return 40;
    if (habits.indexOf("any") >= 0) return 12;
    return 0;
  }

  function matchTriggers(adviceTriggers, inputTriggers) {
    if (!inputTriggers.length) {
      return { score: adviceTriggers.length ? 4 : 0, matches: [] };
    }

    var matches = inputTriggers.filter(function (tag) {
      return adviceTriggers.indexOf(tag) >= 0;
    });
  ...
  function matchRisk(riskLevels, riskLevel) {
    if (!riskLevel || !riskLevels || !riskLevels.length) return 0;
    return riskLevels.indexOf(riskLevel) >= 0 ? 10 : 0;
  }

  function matchTime(timeSegments, timeSegment) {
    if (!timeSegments || !timeSegments.length || !timeSegment) return 0;
    return timeSegments.indexOf("any") >= 0 || timeSegments.indexOf(timeSegment) >= 0 ? 8 : 0;
  }

  function verificationPenalty(entry, includeUnreviewed) {
    if (includeUnreviewed) return 0;
    return entry.verificationStatus === "needs_review" ? -1000 : 0;
  }

  function scoreAdvice(entry, input) {
    var triggerTags = normalizeTriggers(input.triggerTags);
    var habitScore = matchHabit(entry.habits, input.habitId);
    var triggerMatch = matchTriggers(entry.triggerTags, triggerTags);
    var score =
      habitScore +
      triggerMatch.score +
  ...
  function matchAdvice(input, limit) {
    return adviceLibrary
      .map(function (entry) { return scoreAdvice(entry, input); })
      .filter(function (item) { return item.score > 0; })
      .sort(function (a, b) { return b.score - a.score; })
      .slice(0, typeof limit === "number" ? limit : 6);
  }

  function buildAdviceBundle(input) {
    var matches = matchAdvice(input, 8);
    var primary = matches.find(function (item) {
      return item.advice.kind === "interrupt" || item.advice.kind === "replacement" || item.advice.kind === "planning";
    }) || matches[0] || null;

    var reflection = matches.find(function (item) {
      return item.advice.kind === "reflection" || item.advice.kind === "recovery";
    }) || null;

    var support = matches.filter(function (item) {
      return item !== primary && item !== reflection;
    }).slice(0, 3);
  ...
  var api = {
    ...
    matchAdvice: matchAdvice,
    buildAdviceBundle: buildAdviceBundle
  };
